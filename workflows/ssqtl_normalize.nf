...
workflow SSQTL_NORMALIZE {
    main:
    def required = [
        'run_id', 'generation_id', 'ssqtl_associations', 'ssqtl_rds_dir',
        'ssqtl_bam_lookup', 'ssqtl_violin_dir', 'ssqtl_input_root',
        'ssqtl_reference', 'ssqtl_reference_root', 'ssqtl_normalization_output',
        'ssqtl_bind_contract', 'runtime_manifest'
    ]
    def missing = required.findAll { key -> params[key] == null || params[key].toString().trim() == '' }
    if (missing) error("SSQTL_NORMALIZE missing required parameters: ${missing.join(', ')}")
    ...
    if (!(manifestSha ==~ /[a-f0-9]{64}/)) error('SSQTL_NORMALIZE requires runtime_manifest_sha256')
    if (!(fingerprintSha ==~ /[a-f0-9]{64}/)) error('SSQTL_NORMALIZE requires runtime_fingerprint_sha256')
    if (ociDigest && !(ociDigest ==~ /sha256:[a-f0-9]{64}/)) error('SSQTL_NORMALIZE observed OCI digest is invalid')
    ...
    VALIDATE_RUNTIME_IDENTITY(
        runtimeManifest, manifestSchema, runtimeMaterials, explicitLocks,
        runtimeConfig, identityValidator, manifestSha, fingerprintSha,
        ociDigest, runtimeSifSha,
    )
    runtimeValidation = VALIDATE_RUNTIME_IDENTITY.out.bundle.first()
    NORMALIZE_SSQTL_V3(
        params.run_id.toString(), params.generation_id.toString(),
        params.ssqtl_associations.toString(), params.ssqtl_rds_dir.toString(),
        params.ssqtl_bam_lookup.toString(), params.ssqtl_violin_dir.toString(),
        params.ssqtl_input_root.toString(), params.ssqtl_reference.toString(),
        (params.ssqtl_config ?: '').toString(), bindContract, runtimeValidation,
        fingerprintSha,
    )
